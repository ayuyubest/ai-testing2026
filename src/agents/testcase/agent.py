...
from agents.testcase.tools import tools

load_dotenv()


# ============================================================================
# 大语言模型配置（延迟初始化以加快启动速度）
# ============================================================================
_llm = None
...
def get_llm():
    ...
    global _llm
    if _llm is None:
        _llm = init_chat_model("deepseek:deepseek-chat")
    return _llm
...
Step 7：调用 save_testcases_to_excel 工具，将所有测试用例导出为Excel文件保存到test_output文件夹
```
...
async def dynamic_model_selection(request: ModelRequest, handler) -> ModelResponse:
    ...
    """
    from core.llms import get_image_llm_model, get_deepseek_model

    if _has_image_in_messages(request):
        # 消息中含有图片，切换为多模态视觉模型
        model = get_image_llm_model()
    else:
